...
public class ESCache<E> {
     ...
     * @return 对应的多个键值
     */
    public Map<E, E> hmget(String key) {
        return (Map<E, E>) redisTemplate.opsForHash().entries(key);
    }
     ...
     */
    public void hdel(String key, E... item) {
     ...
     * @return 成功个数
     */
    public long sSetAndTime(String key, long time, E... values) {
        try {
     ...
     * @return
     */
    public boolean lSet(String key, JSONArray value) {
        try {
     ...
     * @return
     */
    public boolean lSet(String key, List<E> value, long time) {
        try {
            redisTemplate.opsForList().rightPushAll(key, value);
            if (time > 0)
                expire(key, time);
